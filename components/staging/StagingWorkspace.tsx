...
export default function StagingWorkspace({ onCheckoutInitiated }: StagingWorkspaceProps) {
  // File & preview state
  ...
  const startPolling = (
    ...
    pollRef.current = setInterval(async () => {
      try {
        const j = await getJobStatus(jobIdToPoll);
  ...
  const startRender = async () => {
    ...
    try {
      ...
      // Use staging service
      const uploadData = await uploadImage(userId, file.name, fileBase64);
      const imageUrl = uploadData.publicUrl;
      originalImageUrlRef.current = imageUrl;

      setStatusText("Image uploaded. Starting AI staging...");

      const renderData = await createRenderJob(userId, imageUrl, roomType, style);
      ...
      setIsProcessing(false);
      setStatusText(err?.message || "Unexpected error during staging.");
    }
  };
  ...
  const handleRegenerateClick = async (overrideRoomType?: string, overrideStyle?: string) => {
    ...
    try {
      ...
      const newUrl = await requestVariation(getUserId(), renderId, roomTypeValue, styleValue);

      if (newUrl) {
        setVariationUrls((prev) => {
          const seeded = prev.length > 0 ? prev : stagedUrl ? [stagedUrl] : [];
  ...
  const handlePurchaseClick = async () => {
    ...
    try {
      const checkoutUrl = await initiateCheckout(job.id, currentVariationIndex ?? 0);
