...
export default function Index() {
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        ...
        setModalRoomType(finalRoomTypes[0]);
        ...
        setModalRoomType(fallbackRooms[0]);
  ...
  const handleRegenerateClick = async (
    ...
    try {
      ...
      const roomTypeValue = roomOverride || roomType || job.room_type || "living";
      const styleValue = styleOverride || style || job.style || "standard";
  ...
  const openRegenerateModal = () => {
    if (!previewUrl || !stagedUrl || isProcessing) return;
  ...
  const handleRegenerateConfirm = async () => {
    // Sync the main selects with the modal choices
  ...
  // Purchase -> Stripe Checkout (then your backend can email + store download link)
  ...
  const stagedOrPreview = stagedUrl || previewUrl || undefined;
  ...
  return (
    <>
          ...
          >
            ...
            <section className="space-y-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-lg shadow-slate-200/60">
              <div className="space-y-6">
                ...
                {!previewUrl ? (
                ...
                ) : isProcessed && stagedUrl && previewUrl ? (
                  <div className="space-y-4">
                    ...
                    {/* After-staging CTAs under the rendered image */}
              ...
              {previewUrl && (
                <div className="space-y-6">
                  ...
                  {/* Main CTA BEFORE processing is done */}
                  {!isProcessed && (
      ...
      {/* Regenerate modal with restyle options */}
